...
7. 使用吸引人的标题：''',
        ...
        'my_style': '''10. **我的笔记风格**: 严格遵循以下规则：
### 1. 结论前置
每节第一句就是核心结论，不铺垫、不引入。

### 2. 表格替代段落
对比、列举、状态检查优先用表格，不用长段落。两列足以说清的不用三列。

### 3. 命令独立成行
所有可执行的命令单独成行（代码块内），不加引导语。

### 4. 去冗余
- 删除推广/点赞关注、开头引子、主观评价、过渡句、结尾套话
- 删除重复强调（同一个意思说两遍）
- 删除类比/比喻（除非理解核心概念所必需）
